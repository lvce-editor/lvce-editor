...
import * as GetDebPackageArch from '../GetDebPackageArch/GetDebPackageArch.ts'
import * as GetInstalledSize from '../GetInstalledSize/GetInstalledSize.ts'
import * as IsFakeRoot from '../IsFakeRoot/IsFakeRoot.ts'
import * as LinuxDependencies from '../LinuxDependencies/LinuxDependencies.ts'
...
const fixPermissions = async ({ debArch, product }) => {
  try {
    ...
    await Exec.exec('find', [folder, '-type', 'f', '-name', '*.svg', '-perm', '755', '-print', '-exec', 'chmod', '644', '{}', '+'], {
      stdout: 'ignore',
      stderr: 'inherit',
    })
