...
export const bundleWorkers = async ({ commitHash, platform, assetDir, version, date, product, toRoot }) => {
  ...
  await Copy.copyFile({
    ...
    to: Path.join(`${toRoot}/packages/completion-system-worker`, 'dist', 'completionWorkerMain.js'),
  })
  await Copy.copyFile({
    from: 'packages/renderer-worker/node_modules/@lvce-editor/rename-worker/dist/renameWorkerMain.js',
    to: Path.join(`${toRoot}/packages/rename-system-worker`, 'dist', 'renameWorkerMain.js'),
  })
