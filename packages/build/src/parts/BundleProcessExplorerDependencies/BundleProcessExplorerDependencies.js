...
import * as FilterProcessExplorerDependencies from '../FilterProcessExplorerDependencies/FilterProcessExplorerDependencies.js'
...
export const bundleProcessExplorerDependencies = async ({ to, arch, electronVersion, exclude = [], platform = process.platform }) => {
  const projectPath = Path.absolute('packages/process-explorer')
  const npmDependenciesRaw = await NpmDependencies.getNpmDependenciesRawJson(projectPath)
  const npmDependencies = FilterProcessExplorerDependencies.filterDependencies(npmDependenciesRaw, exclude)
