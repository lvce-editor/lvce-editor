...
export const loadContent = async (state) => {
  const savedState = {}
  await ChatDebugViewWorker.invoke('ChatDebug.create', state.uid, '', state.x, state.y, state.width, state.height, state.platform, state.assetDir)
...
export const hotReload = async (state) => {
  ...
  await ChatDebugViewWorker.invoke('ChatDebug.create', state.uid, '', state.x, state.y, state.width, state.height, null)
  await ChatDebugViewWorker.invoke('ChatDebug.loadContent', state.uid, savedState)
  const diffResult = await ChatDebugViewWorker.invoke('ChatDebug.diff2', state.uid)
  const commands = await ChatDebugViewWorker.invoke('ChatDebug.render2', state.uid, diffResult)
