...
export const loadContent = async (state: ChatState, savedState: unknown): Promise<ChatState> => {
  ...
  await ChatViewWorker.invoke('Chat.create', id, x, y, width, height, state.platform)
...
export const hotReload = async (state) => {
  ...
  await ChatViewWorker.invoke('Chat.create', state.uid, state.x, state.y, state.width, state.height, state.platform)
