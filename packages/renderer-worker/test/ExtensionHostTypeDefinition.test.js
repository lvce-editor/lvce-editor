...
test('executeTypeDefinitionProvider', async () => {
  ...
  expect(await ExtensionHostTypeDefinition.executeTypeDefinitionProvider({ id: 1, uri: '' }, 0)).toEqual([])
})
...
test('executeTypeDefinitionProvider - error', async () => {
  ...
  await expect(ExtensionHostTypeDefinition.executeTypeDefinitionProvider({ id: 1, uri: '' }, 0)).rejects.toThrow(new TypeError('x is not a function'))
})
