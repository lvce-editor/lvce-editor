...
test.skip('show - multiple times and out of order promises', async () => {
  ...
  expect(RendererProcess.invoke).toHaveBeenCalledTimes(1)
  expect(RendererProcess.invoke).toHaveBeenCalledWith(3028, 'ImagePreview', '/image-2.png', -12, -160)
})
...
test.skip('show - error', async () => {
  ...
  expect(RendererProcess.invoke).toHaveBeenCalledTimes(1)
  expect(RendererProcess.invoke).toHaveBeenCalledWith(3028, 'ImagePreview', 'Image could not be loaded', -12, -160)
})
...
test('hide', async () => {
  ...
  expect(RendererProcess.invoke).toHaveBeenCalledWith('Viewlet.dispose', 'ImagePreview')
})
