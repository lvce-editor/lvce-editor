...
const ExtensionHostHover = await import('../src/parts/ExtensionHost/ExtensionHostHover.js')
...
test('executeHoverProvider - no result', async () => {
  ...
  expect(await ExtensionHostHover.executeHoverProvider({ id: 1, languageId: 'test' }, 0)).toEqual({})
...
test('executeHoverProvider - error - hoverProvider throws error', async () => {
  ...
  await expect(ExtensionHostHover.executeHoverProvider({ id: 1, languageId: 'test' }, 0)).rejects.toThrow(new TypeError('x is not a function'))
})
