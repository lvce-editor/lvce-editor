...
test('readFile', async () => {
  ...
  expect(
    await FileSystemWeb.readFile('web', 'web:///workspace/languages/index.dart')
  ).toEqual(
    `void main() {
  print('Hello, World!');
}`
...
test('readFile - error', async () => {
  // TODO passing protocol here seems unnecessary, but it is useful for extension host which has several protocols
  await expect(
    FileSystemWeb.readFile('web', 'web:///languages/index.dart')
  ).rejects.toThrowError(new Error('file not found'))
})

test('rename', async () => {
  expect(
    FileSystemWeb.rename('web', '/tmp/some-file.txt', '/tmp/renamed.txt')
