...
test('create', () => {
  ...
  expect(state.$ImagePreviewImage.src).toBe('http://localhost/remote/tmp/image.png')
...
test('create - error with loading image', () => {
  ...
  expect(state.$ImagePreview.isConnected).toBe(true)
  expect(state.$ImagePreviewImage.src).toBe('http://localhost/remote/tmp/image.png')
  state.$ImagePreviewImage.dispatchEvent(new ErrorEvent('error', { bubbles: true }))
  expect(state.$ImagePreviewCaption.textContent).toBe('Image could not be loaded')
})
