...
test('VError - merging stacks - parent stack does not include message', () => {
  ...
  expect(verror.message).toBe('Failed to open about window: Error: Unknown command "ElectronWindowAbout.open"')
  expect(verror.stack).toBe(
...
test('VError - merging stacks - parent has no stack but has line number and column number', () => {
  ...
  expect(verror.message).toBe("Failed to load module: SyntaxError: expected expression, got ','")
  expect(verror.stack).toMatch('  at http://localhost:3000/packages/renderer-process/src/parts/TestFrameWork/ElementActions.js:17:47')
})
