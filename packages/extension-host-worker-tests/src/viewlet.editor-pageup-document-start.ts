...
export const test: Test = async ({ Command, Editor, FileSystem, KeyBoard, Locator, Main, Workspace, expect }) => {
  ...
  await expect(cursor).toBeVisible()
  await expect(cursor).toHaveCSS('translate', '0px 0px')
  await expect(Locator('.EditorRow').first()).toContainText('line 00')

  await KeyBoard.press('PageUp')
  await expect(cursor).toBeVisible()
  await expect(cursor).toHaveCSS('translate', '0px 0px')
  if ((await Command.execute('FileSystem.readFile', filePath)) !== lines.join('\n')) {
  ...
  await expect(Locator('.EditorCursor')).toHaveCSS('translate', '0px 0px')
}
