...
export const test = async ({ Extension, QuickPick, Locator, expect }) => {
  ...
  await expect(errorMessage).toHaveText(
    `Object: Failed to activate extension sample.error-clone: DataCloneError: Failed to execute 'structuredClone' on 'WorkerGlobalScope': #<Promise> could not be cloned.`,
  )
