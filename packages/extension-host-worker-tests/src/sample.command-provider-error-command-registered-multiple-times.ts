...
export const test = async ({ Extension, QuickPick, Locator, expect }) => {
  ...
  await expect(dialogErrorMessage).toHaveText(
    'Failed to activate extension sample.command-provider-error-command-registered-multiple-times: Failed to register command xyz.sampleCommand: command cannot be registered multiple times',
  )
}
