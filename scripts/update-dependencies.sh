...
function updateDependencies {
  ...
  OUTPUT=`ncu -u -x msw -x @types/node -x rollup -x electron-unhandled -x electron -x execa -x electron-builder -x jest -x jest-environment-jsdom -x lerna -x typescript -x @babel/preset-typescript`
